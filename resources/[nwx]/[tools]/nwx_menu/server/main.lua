...
ESX.RegisterServerCallback('esx_cinematique:getItemAmount', function(source, cb, item)
...
function getMaximumGrade2(factionname)
	local result = MySQL.Sync.fetchAll("SELECT * FROM faction_grades WHERE faction_name = @factionname ORDER BY `grade` DESC ;", {
...
AddEventHandler('KorioZ-PersonalMenu:Boss_promouvoirplayer2', function(target)
	...
	local sourceXPlayer = ESX.GetPlayerFromId(_source)
	local targetXPlayer = ESX.GetPlayerFromId(target)
	local maximumgrade = tonumber(getMaximumGrade2(sourceXPlayer.faction.name)) -1
...
AddEventHandler('KorioZ-PersonalMenu:Boss_recruterplayer2', function(target, faction, grade2)
	local _source = source
