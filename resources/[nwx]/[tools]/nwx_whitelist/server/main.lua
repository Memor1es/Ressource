...
function loadWhiteList()
	MySQL.Async.fetchAll('SELECT identifier FROM whitelist', {},
		function (result)
			WhiteList = {}
			for i=1, #result, 1 do
...
TriggerEvent('es:addGroupCommand', 'addwl', 'admin', function (source, args, user)
	if not args[1] or not args[2] then
		TriggerEvent('esx_whitelistExtended:sendMessage', source, '^1SYSTEM', 'Usage invalide')
		return
	end
	...
	if args[1] == 'hex' then
		if string.len(args[2]) == 21 then
			...
			TriggerEvent('esx_whitelistExtended:sendMessage', source, '^1SYSTEM', 'Longueur du steam hexadécimal invalide')
		end
	elseif args[1] == 'dec' then
		if tonumber(args[2]) and string.len(args[2]) == 17 then
			...
			TriggerEvent('esx_whitelistExtended:sendMessage', source, '^1SYSTEM', 'Longueur du steam décimal invalide')
		end
	else
		TriggerEvent('esx_whitelistExtended:sendMessage', source, '^1SYSTEM', 'Utilisation invalide')
	end
end, function (source, args, user)
	TriggerClientEvent('chat:addMessage', source, { args = { '^1SYSTEM', 'Insufficienct permissions!' } })
end, {help = "Ajouté un joueur a la whitelist", params = {{name = "numeral system", help = "mettez ici 'hex' (hexadécimal) ou 'dec' (décimal)"}, {name = "steam identifier", help = "l'identifiant steam, soit en décimal soit un hex"}}})
...
AddEventHandler('esx_whitelistExtended:whitelistUser', function(source, identifier)
	MySQL.Async.fetchAll('SELECT * FROM whitelist WHERE identifier=@identifier', {['@identifier'] = identifier}, function(result)
		if result[1] ~= nil then
			TriggerEvent('esx_whitelistExtended:sendMessage', source, '^1SYSTEM', 'Le joueur est déjà whitelist')
		else
			MySQL.Async.execute("INSERT INTO whitelist (identifier) VALUES (@identifier)", {['@identifier'] = identifier})
			TriggerEvent('esx_whitelistExtended:sendMessage', source, 'Whitelist', 'Le joueur a été whitelist Identifiant : ' .. identifier)
			loadWhiteList()
		end
	end)
end)
