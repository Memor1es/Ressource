    ...
    for i = 1, #Config.Zones[zone].Items, 1 do
        ...
        if Config.Zones[zone].Free then
            ...
            if zone == "BikeRental" and RentedBike then
                table.insert(elements, {
                    label = ('%s - <span style="color: #B01010;">%s</span>'):format(item.label, _U('item_price', Config.CurrencyPrefix, ESX.Math.GroupDigits(item.price * Config.Zones[zone].Multiplier), Config.CurrencySuffix)),
                    item = item.item,
                    action = Config.Zones[zone].MenuActions.MenuTrigger,
                    itemId = i,
                    itemType = Config.Zones[zone].ItemType,
                ...
                table.insert(elements, {
                    label = ('%s - <span style="color: #B01010;">%s</span>'):format(item.label, _U('item_price', Config.CurrencyPrefix, ESX.Math.GroupDigits(item.price), Config.CurrencySuffix)),
                    item = item.item,
                    action = Config.Zones[zone].MenuActions.MenuTrigger,
                    itemId = i,
                    itemType = Config.Zones[zone].ItemType,
