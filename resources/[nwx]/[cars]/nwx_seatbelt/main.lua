...
Fwv = function (entity)
    ...
    if car ~= 0 and (wasInCar or IsCar(car)) then
      wasInCar = true
        ...
        if speedBuffer[2] ~= nil and 
          not beltOn and 
          GetEntitySpeedVector(car, true).y > 10.0 and 
          speedBuffer[1] > 10.25 and 
          (speedBuffer[2] - speedBuffer[1]) > (speedBuffer[1] * 0.255) then
        ...
        if beltOn then 
		  ...
		  isUiOpen = true
      ShowNotification("Ceinture de sécurité ~g~attachée~.") 
		else 
		  SendNUIMessage({
		    displayWindow = 'true'
		    })
		  isUiOpen = true
		  ShowNotification("Ceinture de sécurité ~r~Détachée~.") 
		end
      end
...
Citizen.CreateThread(function()
  while true do
    ...
    if not beltOn and wasInCar == true then
      TriggerServerEvent('InteractSoundS:PlayWithinDistance', 0.1, 'seatbelt', 0.1)
      Citizen.Wait(5000)
    end
	end
end)