...
local function Sell(source, zone)
	...
	if PlayersSelling[source] == true then
		...
		if zone == 'SellFarm' then
			...
			if vin == 0 and jus == 0 then
				xPlayer.showNotification(_U('no_product_sale'))
				return
			elseif xPlayer.getInventoryItem('vin').count <= 0 and jusraisin == 0 then
				xPlayer.showNotification(_U('no_vin_sale'))
				vin = 0
				return
			elseif xPlayer.getInventoryItem('jusraisin').count <= 0 and vin == 0then
				...
				if (jusraisin == 1) then
					SetTimeout(1100, function()
						...
						if societyAccount ~= nil then
							...
							xPlayer.addMoney(money2)
							TriggerClientEvent('esx:showNotification', xPlayer.source, _U('player_earned') .. money2)
						end
						Sell(source,zone)
					end)
				elseif (vin == 1) then
