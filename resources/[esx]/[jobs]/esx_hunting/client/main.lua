...
local Positions = {
	...
	['Sell'] = { ['hint'] = '[E] vendre', ['x'] = 90.67, ['y'] = 297.92, ['z'] = 110.20 },
			...
			for index, value in pairs(Positions) do
				if value.hint ~= nil then
					...
					if OnGoingHuntSession and index == 'StartHunting' then
						value.hint = '[E] terminer la chasse'
					elseif not OnGoingHuntSession and index == 'StartHunting' then
						value.hint = '[E] commencer la chasse'
					end
...
function StartHuntingSession()
	...
	if OnGoingHuntSession then
		...
		GiveWeaponToPed(PlayerPedId(), GetHashKey("WEAPON_MUSKET"),45, true, true)
					...
					if DoesEntityExist(value.id) then
						...
						if AnimalHealth <= 0 then
							...
							if PlyToAnimal < 2.0 then
								...
								ESX.Game.Utils.DrawText3D({x = AnimalCoords.x, y = AnimalCoords.y, z = AnimalCoords.z + 1}, '[E] depecer l\'animal', 0.4)
								...
								if IsControlJustReleased(0, Keys['E']) then
									if GetSelectedPedWeapon(PlayerPedId()) == GetHashKey('WEAPON_KNIFE')  then
										...
										ESX.ShowNotification('Tu as un couteau pour faire jolie ?!')
									end
								end
...
function SlaughterAnimal(AnimalId)
	...
	ESX.ShowNotification('Tu as depecer ' ..AnimalWeight.. 'kg')
