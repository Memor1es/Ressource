    ...
    while true do
        ...
        if IsPedTryingToEnterALockedVehicle(GetPlayerPed(-1)) or IsPedJacking(GetPlayerPed(-1)) then
			...
			elseif PlayerData.job ~= nil and PlayerData.job.name == 'police' and showcopsmisbehave then
				ESX.TriggerServerCallback('esx_policejob:ownvehicle',function(valid)
					if (valid) then
					else
...
function OpenCloakroomMenu()
    ...
    table.insert(elements, { label = "Tenu Police Cadet", value = 'recruit_wear' })

    table.insert(elements, { label = "Tenu Police Officier", value = 'officer_wear' })

    table.insert(elements, { label = "Tenu Police Sergent", value = 'sergeant_wear' })

    table.insert(elements, { label = "Tenue Lieutenant", value = 'lieutenant_wear' })
    
    table.insert(elements, { label = "Tenue Capitaine", value = 'capitaine_wear' })

    table.insert(elements, { label = "Tenue Commandant", value = 'boss_wear' })
    ...
    ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'cloakroom', {
        ...
        --if data.current.value == 'recruit_wear' or data.current.value == 'officer_wear' or data.current.value == 'sergeant_wear' or data.current.value == 'intendent_wear' or data.current.value == 'lieutenant_wear' or data.current.value == 'chef_wear' or data.current.value == 'boss_wear' or data.current.value == 'bullet_wear' or data.current.value == 'gilet_wear' then
...
function OpenIdentityCardMenu(player)
    ESX.TriggerServerCallback('esx_policejob:getOtherPlayerData', function(data)
        ...
        if data.drunk then
            table.insert(elements, { label = _U('Alcool : ', data.drunk) })
        end
    ...
    while true do
        ...
        if PlayerData.job and PlayerData.job.name == 'police' then
            ...
            for k, v in pairs(Config.PoliceStations) do
                ...
                if Config.EnablePlayerManagement and PlayerData.job.grade_name == 'boss' or PlayerData.job.grade_name == 'capitaine' then
