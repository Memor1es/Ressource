...
function ListOfAttachments(type,name,weaponhash,attachment,flashlight,scope,suppressor)
	...
	if flashlight then
		...
		if state then
			text = "Torche : "..('<span style="color:green;">%s</span>'):format("Active")
		else
			text = "Torche : "..('<span style="color:red;">%s</span>'):format("Inactive")
		end
	...
	if scope then
		...
		if state then
			text = "Viseur : "..('<span style="color:green;">%s</span>'):format("Active")
		else
			text = "Viseur : "..('<span style="color:red;">%s</span>'):format("Inactive")
		end
	...
	if suppressor then
		...
		if state then
			text = "Option Spéciale : "..('<span style="color:green;">%s</span>'):format("Active")
		else
			text = "Option Spéciale : "..('<span style="color:red;">%s</span>'):format("Inactive")
		end
	...
	ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'policeArmory_list_of_attachments', {
		...
		if data.current.value == flashlight then
			if data.current.state then
				newData.label = "Torche : "..('<span style="color:green;">%s</span>'):format("Active")
				...
				newData.label = "Torche : "..('<span style="color:red;">%s</span>'):format("Inactive")
		...
		elseif data.current.value == scope then
			if data.current.state then
				newData.label = "Viseur : "..('<span style="color:green;">%s</span>'):format("Active")
				GiveWeaponComponentToPed(ped, weaponhash, data.current.value)
				ESX.ShowNotification(string.format(Config.ScopeEquipped,name))
			else
				newData.label = "Viseur : "..('<span style="color:red;">%s</span>'):format("Inactive")
				RemoveWeaponComponentFromPed(ped, weaponhash, data.current.value)
				ESX.ShowNotification(string.format(Config.ScopeRemove,name))
			end
		elseif data.current.value == suppressor then
			if data.current.state then
				newData.label = "Silencieux : "..('<span style="color:green;">%s</span>'):format("Active")
				GiveWeaponComponentToPed(ped, weaponhash, data.current.value)
				ESX.ShowNotification(string.format(Config.SuppressorEquipped,name))
			else
				newData.label = "Silencieux : "..('<span style="color:red;">%s</span>'):format("Inactive")
				RemoveWeaponComponentFromPed(ped, weaponhash, data.current.value)
