...
function drawLevel(r, g, b, a)
	...
	--Affichage Classique constamment visible
	BeginTextCommandDisplayText("STRING")
	AddTextComponentSubstringPlayerName(_U('voice', voice.level))
	EndTextCommandDisplayText(0.256, 0.944)
end
	...
	while true do
		...
		if IsControlJustPressed(1, Keys[',']) then
			...
			if voice.current == 0 then
				...
				ESX.ShowNotification(_U('younormal'))

			elseif voice.current == 1 then
				NetworkSetTalkerProximity(voice.shout)
				voice.level = _U('shout')
				ESX.ShowNotification(_U('youshout'))

			elseif voice.current == 2 then
				NetworkSetTalkerProximity(voice.whisper)
				voice.level = _U('whisper')
				ESX.ShowNotification(_U('youwhisper'))
			end
		end
