...
function OpenBossMenuFaction(society, close, options)
	...
	ESX.TriggerServerCallback('esx_societyfaction:isBossFaction', function(result)
		isBoss = result
	end, society)
	...
	ESX.TriggerServerCallback('esx_societyfaction:getEmployeesFaction', function(employees)
		...
		ESX.UI.Menu.Open('list', GetCurrentResourceName(), 'employee_list_' .. society, elements, function(data, menu)
			...
			elseif data.value == 'fire' then
				...
				end, employee.identifier, 'resid', 0, 'fire')
			end
		end, function(data, menu)
			menu.close()
			OpenManageEmployeesMenuFaction(society)
...
AddEventHandler('esx_societyfaction:openBossMenuFaction', function(society, close, options)
