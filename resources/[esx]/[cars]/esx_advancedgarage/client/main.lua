...
function ListOwnedAmbulanceMenu()
	...
	if Config.ShowVehicleLocation and Config.ShowSpacers then
		local spacer = ('| <span>%s</span> - <span>%s</span> - <span>%s</span> |'):format(_U('plate'), _U('vehicle'), _U('location'))
		table.insert(elements, {label = spacer, value = nil})
	elseif Config.ShowVehicleLocation == false and Config.ShowSpacers then
		local spacer = ('| <span>%s</span> - <span>%s</span> |'):format(_U('plate'), _U('vehicle'))
		table.insert(elements, {label = ('<span>%s</span>'):format(_U('spacer1')), value = nil})
		table.insert(elements, {label = spacer, value = nil})
	end
	...
	ESX.TriggerServerCallback('esx_advancedgarage:getOwnedAmbulanceCars', function(ownedAmbulanceCars)
		if #ownedAmbulanceCars == 0 then
			...
			for _,v in pairs(ownedAmbulanceCars) do
				...
				local plate = v.plate
				local labelvehicle
				local labelvehicle2 = ('| <span>%s</span> - <span>%s</span> - '):format(plate, vehicleName)
				local labelvehicle3 = ('| <span>%s</span> - <span>%s</span> | '):format(plate, vehicleName)

				if Config.ShowVehicleLocation then
					if v.stored then
						labelvehicle = labelvehicle2 .. ('<span>%s</span> |'):format(_U('loc_garage'))
					else
						labelvehicle = labelvehicle2 .. ('<span>%s</span> |'):format(_U('loc_pound'))
					end
				else
					if v.stored then
	...
	if WasinJPound then
		...
		ESX.TriggerServerCallback('esx_advancedgarage:getOutOwnedAmbulanceCars', function(ownedAmbulanceCars)
			...
			if Config.ShowVehicleLocation == false and Config.ShowSpacers then
				local spacer = ('| <span style="color:red;">%s</span> - <span>%s</span> |'):format(_U('plate'), _U('vehicle'))
				table.insert(elements, {label = spacer, value = nil})
			end
			...
			for _,v in pairs(ownedAmbulanceCars) do
				...
				local plate = v.plate
				local labelvehicle
				local labelvehicle2 = ('| <span style="color:red;">%s</span> - <span>%s</span> - '):format(plate, vehicleName)
...
function ListOwnedPoliceMenu()
	...
	if Config.ShowVehicleLocation and Config.ShowSpacers then
		local spacer = ('| <span style="color:red;">%s</span> - <span>%s</span> - <span style="color:red;">%s</span> |'):format(_U('plate'), _U('vehicle'), _U('location'))
		table.insert(elements, {label = spacer, value = nil})
	elseif Config.ShowVehicleLocation == false and Config.ShowSpacers then
		local spacer = ('| <span style="color:red;">%s</span> - <span>%s</span> |'):format(_U('plate'), _U('vehicle'))
	...
	ESX.TriggerServerCallback('esx_advancedgarage:getOwnedPoliceCars', function(ownedPoliceCars)
		if #ownedPoliceCars == 0 then
			...
			for _,v in pairs(ownedPoliceCars) do
				...
				local labelvehicle
				local labelvehicle2 = ('| <span style="color:red;">%s</span> - <span>%s</span> - '):format(plate, vehicleName)
				local labelvehicle3 = ('| <span style="color:red;">%s</span> - <span>%s</span> | '):format(plate, vehicleName)
	...
	if WasinJPound then
		...
		ESX.TriggerServerCallback('esx_advancedgarage:getOutOwnedPoliceCars', function(ownedPoliceCars)
			...
			for _,v in pairs(ownedPoliceCars) do
				...
				labelvehicle = labelvehicle2 .. ('<span>%s</span> |'):format(_U('return'))
...
function ListOwnedBoatsMenu()
	...
	if Config.ShowVehicleLocation and Config.ShowSpacers then
		local spacer = ('| <span style="color:red;">%s</span> - <span>%s</span> - <span>%s</span> |'):format(_U('plate'), _U('vehicle'), _U('location'))
		table.insert(elements, {label = spacer, value = nil})
	elseif Config.ShowVehicleLocation == false and Config.ShowSpacers then
		local spacer = ('| <span>%s</span> - <span>%s</span> |'):format(_U('plate'), _U('vehicle'))
		table.insert(elements, {label = ('<span>%s</span>'):format(_U('spacer1')), value = nil})
