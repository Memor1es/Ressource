(function(){
    ...
    if(tool == 'edit'){
      ...
      $.post('http://es_mapper/editor.get_object_infos', '{}').success(function(data){
    ...
    if(tool == 'list'){
      ...
      $('.object-tree-container').fancytree('getTree').reload({
        type: 'POST',
        url: 'http://es_mapper/editor.get_nodes',
        data: '{}'
      });
    ...
    $.post('http://es_mapper/editor.select_tool', JSON.stringify({
      tool: $(this).data('tool')
    }));
	});
  ...
  $('.action[data-action]').click(function(){
    ...
    .change(function(){
      ...
      $.ajax({
        method: 'POST',
        url: "http://es_mapper/editor.get_objects",
        dataType: 'json',
        data: JSON.stringify({
          q: $(this).val()
        }),
  ...
  $('select[name="object-list"]')
    ...
    .change(function(){
      ...
      $.post('http://es_mapper/editor.tool.object.select', JSON.stringify({
        name   : $(this).val(),
  ...
  $('input[name="position-x"]')
    .on('spin', function(event, ui){
      $.post('http://es_mapper/editor.set_object_position', JSON.stringify({
  ...
  $('input[name="rotation-x"]')
    .on('spin', function(event, ui){
      $.post('http://es_mapper/editor.set_object_rotation', JSON.stringify({
        x: parseFloat(ui.value),
  ...
  $('input[name="object-freeze"]').change(function(){
    ...
    $.post('http://es_mapper/editor.set_object.freeze', JSON.stringify({
      freeze: checked
    }));
  ...
  $('input[name="object-dynamic"]').change(function(){
    ...
    $.post('http://es_mapper/editor.set_object.dynamic', JSON.stringify({
      dynamic: checked
    }));
    ...
    contextMenu: {
      ...
      actions: function(node, action, options) {
        ...
        switch(action) {
          ...
          case 'select' : {
            $.post('http://es_mapper/editor.select_node', JSON.stringify({
          ...
          case 'delete' : {
            $.post('http://es_mapper/editor.delete_node', JSON.stringify({
              id : parseInt(node.key)
            })).success(function(data){
              $('.object-tree-container').fancytree('getTree').reload({
                type: 'POST',
  ...
  $('#rename-dialog input[name="name"]').keyup(function(event){
    if(event.which == 13){
      ...
      $.post('http://es_mapper/editor.rename_node', JSON.stringify({
  ...
  $('#open-dialog input[name="name"]').keyup(function(event){
    if(event.which == 13){
      ...
      $.post('http://es_mapper/editor.action', JSON.stringify({
        action: 'open',
        file  : file
      }));
  ...
  document.onkeyup = function(event) {
    ...
    if(event.which == 27 || (event.which == 82 && $(':focus').length == 0) ) {
      $.post('http://es_mapper/editor.hide_menu', '{}');
      $('.input-dialog').hide();
    }
