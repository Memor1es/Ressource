...
TriggerEvent('es:addGroupCommand', 'setfaction', 'jobmaster', function(source, args, user)
	if tonumber(args[1]) and args[2] and tonumber(args[3]) then
		local xPlayer = ESX.GetPlayerFromId(args[1])
		...
		if xPlayer then
			if ESX.DoesFactionExist(args[2], args[3]) then
				...
				TriggerClientEvent('chat:addMessage', source, { args = { '^1SYSTEM', 'That faction does not exist.' } })
			end
