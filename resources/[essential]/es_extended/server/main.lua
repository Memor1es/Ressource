AddEventHandler('es:playerLoaded', function(source, _player)
	...
	TriggerEvent('es:getPlayerFromId', _source, function(player)
		...
		table.insert(tasks, function(cb)
			...
			-- Get job/faction name, grade and last position
			table.insert(tasks2, function(cb2)

				MySQL.Async.fetchAll('SELECT faction, faction_grade FROM users WHERE identifier = @identifier', {
					['@identifier'] = player.getIdentifier()
				}, function(result)
					local faction, grade = result[1].faction, tostring(result[1].faction_grade)
					...
					if ESX.DoesFactionExist(faction, grade) then
						...
						userData.faction.grade = tonumber(grade)
						userData.faction.grade_name = gradeObject.name
						userData.faction.grade_label = gradeObject.label
						userData.faction.grade_salary = gradeObject.salary

						userData.faction.skin_male = {}
						userData.faction.skin_female = {}

						if gradeObject.skin_male ~= nil then
							userData.faction.skin_male = json.decode(gradeObject.skin_male)
						end
			
						if gradeObject.skin_female ~= nil then
							userData.faction.skin_female = json.decode(gradeObject.skin_female)
						end
					else
						print(('es_extended: %s had an unknown faction [faction: %s, grade: %s], setting as resid!'):format(player.getIdentifier(), faction, grade))

						local faction, grade = 'resid', '0'
						local factionObject, gradeObject = ESX.Factions[faction], ESX.Factions[faction].grades[grade]
...
AddEventHandler('esx:giveInventoryItem', function(target, type, itemName, itemCount)
	...
	if type == 'item_standard' then
		...
		if itemCount > 0 and sourceItem.count >= itemCount then
			...
			if targetItem.limit ~= -1 and (targetItem.count + itemCount) > targetItem.limit then
				...
				TriggerEvent("esx:giveitemalert",sourceXPlayer.name,targetXPlayer.name,ESX.Items[itemName].label,itemCount) --Discord Bot
	...
	elseif type == 'item_money' then
		...
		if itemCount > 0 and sourceXPlayer.getMoney() >= itemCount then
			...
			TriggerEvent("esx:givemoneyalert",sourceXPlayer.name,targetXPlayer.name,itemCount) --Discord BOT
		else
	...
	elseif type == 'item_account' then
		...
		if itemCount > 0 and sourceXPlayer.getAccount(itemName).money >= itemCount then
			...
			TriggerEvent("esx:givemoneybankalert",sourceXPlayer.name,targetXPlayer.name,itemCount) -- Discord Bot
		else
			TriggerClientEvent('esx:showNotification', _source, _U('imp_invalid_amount'))
		end
	...
	elseif type == 'item_weapon' then
		...
		if not targetXPlayer.hasWeapon(itemName) then
			...
			if itemCount > 0 then
				...
				TriggerEvent("esx:giveweaponalert",sourceXPlayer.name,targetXPlayer.name,weaponLabel) --Discord Bot
			else
