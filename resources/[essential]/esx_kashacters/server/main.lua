...
local IdentifierTables = {
    ...
    {table = 'faction_moneywash', column = 'identifier'},
...
AddEventHandler('kashactersS:CharacterChosen', function(charid, ischar)
    ...
    if ischar == 'true' then
        ...
        spawn = { x = -1045.05, y = -2750.32, z = 21.3634 } -- DEFAULT SPAWN POSITION
    end
...
function GetPlayerCharacters(source)
  ...
  for i = 1, #Chars, 1 do
    ...
    -- Faction
    charFaction = MySQLAsyncExecute("SELECT * FROM `factions` WHERE `name` = '"..Chars[i].faction.."'")
    charFactiongrade = MySQLAsyncExecute("SELECT * FROM `faction_grades` WHERE `grade` = '"..Chars[i].faction_grade.."'")
    Chars[i].faction = charFaction[1].label
    Chars[i].faction_grade = charFactiongrade[1].label
  end
  return Chars
end
